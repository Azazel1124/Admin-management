Вижу проблему. В моей версии я экранировал тройные обратные кавычки символом \ (как ````), потому что внутри markdown-блока сами по себе нельзя писать тройные кавычки — они закроют блок. Но при копировании в реальный README надо убрать эти бэкслеши.

То есть в файле должно быть просто ```, без \. Из-за бэкслешей у тебя GitHub/GitLab видит мусор и не открывает блоки кода правильно — отсюда схема репозитория ломается в одну строку.
Исправь файл

Открой README:
bash

$EDITOR ~/.ansible/ansible-admins/README.md

Используй find&replace в редакторе — замени все вхождения:
...
(убрать обратный слеш перед каждым тройным бэктиком)

В VS Code: Ctrl+H → ввести `\```` в поле поиска (нажми кнопку `.*` для regex, или экранируй) → заменить на ` ``` `.

В Vim: `:%s/\\```/```/g`

В nano: Ctrl+\ → `\`\`\`` → `\`\`\``.

## Или возьми чистую версию ниже

Скопируй целиком, она уже без экранирования:
...
1. Человек у себя генерит ключ: `ssh-keygen -t ed25519 -C "user@machine"`
2. Шлёт публичную часть (`~/.ssh/id_ed25519.pub`) ответственному
3. Ответственный кладёт в `files/admins/admin_<name>.pub`
4. Добавляет запись в `inventory/group_vars/all/admins.yml`
5. Коммит → MR → merge → прогон playbook'а
6. Человек ходит на ноды как `admin_<name>`
...
| Таска | Что делает |
|---|---|
| Validate all pubkeys exist on controller | Проверяет, что для каждого `state: present` админа есть .pub файл |
| Update apt cache | `apt-get update` если кеш старше часа |
| Ensure hostname resolves locally | Добавляет в `/etc/hosts` запись `127.0.1.1 <fqdn> <hostname>` (убирает sudo warning'и) |
| Ensure sudo is installed | Ставит пакет `sudo` (на минимальных VPS его нет) |
| Set safe default TERM | Кладёт скрипт-фолбэк в `/etc/profile.d/` для терминалов вроде kitty/alacritty |
| Create admin users | `useradd` с залоченным паролем, shell `/bin/bash`, домашка создаётся |
| Harden home permissions | `chmod 0700` на `/home/<user>` |
| Install authorized_key | Кладёт публичный ключ из `files/admins/<user>.pub`, `exclusive: true` |
| Grant sudo via dropin | Создаёт `/etc/sudoers.d/10-<user>` с NOPASSWD ALL, валидируется через `visudo -cf` |
| Remove sudoers dropin | Для `state: absent` — удаляет sudoers файл |
| Remove departed admin users | Для `state: absent` — `userdel -r -f` (с домашкой) |
| Verify sudo works | Проверяет что `sudo -u <user> -n true` возвращает 0 |

### harden_ssh.yml

| Таска | Что делает |
|---|---|
| Verify admin_oleg can sudo | Pre-check: если admin_oleg не работает — playbook падает, root не закрывается |
| Verify authorized_keys exists | Pre-check: проверяет что ключи реально лежат у admin_oleg |
| Backup sshd_config | Копия в `/etc/ssh/sshd_config.bak.YYYY-MM-DD` (один раз в день) |
| Set PermitRootLogin no | Закрывает root SSH |
| Set PasswordAuthentication no | Закрывает парольный логин |
| Set KbdInteractiveAuthentication no | Закрывает PAM challenge-response |
| Find sshd_config.d override files | Ищет файлы в `/etc/ssh/sshd_config.d/` |
| Neutralize cloud-init overrides | Комментирует `PasswordAuthentication yes` в файлах провайдера |
| Handler: restart sshd | Применяет конфиг через `systemctl restart ssh` |
...
1. Сгенерировать новую пару у пострадавшего
2. Заменить `files/admins/admin_<name>.pub` на новый
3. Прогнать `add_admins.yml` — `exclusive: true` затрёт старый ключ новым
4. Старая пара теперь бесполезна
...
- **Vault для секретов** — токены Telegram-уведомлений, API-ключи
